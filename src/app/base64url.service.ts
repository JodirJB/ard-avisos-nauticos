...
export class Base64UrlService {
   ...
   */
  encode(input: string, encoding: Encoding = 'ascii'): string {
    if (!input) {
      return '';
    }

    // Remove non-ascii characters if provided character encoding is ascii
    const valueToEncode = encoding === 'ascii' ? input.replace(/[^\x00-\x7F]/g, '') : input;
    ...
    switch (encoding) {
      ...
      case 'utf8':
        const base64 = btoa(
          encodeURIComponent(valueToEncode).replace(/%([0-9A-F]{2})/g,
          (_, p1) => String.fromCharCode(parseInt('0x' + p1, 16)))
        );
        return this.fromBase64(base64);
      case 'hex':
        const hexMatches = valueToEncode.match(/\w{2}/g);
        return this.fromBase64(btoa(hexMatches ? hexMatches.map((a) => String.fromCharCode(parseInt(a, 16))).join('') : ''));
    }
  }
  ...
  decode(input: string, encoding: Encoding = 'ascii'): string {
    ...
    switch (encoding) {
      ...
      case 'hex':
        return atob(base64).split('').map(c => ('0' + c.charCodeAt(0).toString(16)).slice(-2)).join('');
    }
  }
